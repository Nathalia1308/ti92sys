...
namespace ti92class
{
    public class ItemPedido
    {
        ...
        public static List<ItemPedido> Listar(int pedido_id)
        {
            ...
            while (dr.Read())
            {
                itens.Add(new ItemPedido(dr.GetInt32(0), Produto.ObterPorId(dr.GetInt32(2)),dr.GetDouble(3),dr.GetDouble(4),dr.GetDouble(5)));
            }
            return itens;
        }
        ...
        {
            var cmd = Banco.Abrir();
            cmd.CommandText = "insert itempedido (pedido_id, produto_id, preco, quantidade, desconto) values (@pedido_id, @produto_id, @preco, @quantidade, @desconto)";
        ...
        {
            var cmd = Banco.Abrir();
            cmd.CommandText = "update itempedido set quantidade = @quantidade, desconto = @desconto where id = @id and pedido_id = @pedido_id";
            cmd.Parameters.Clear();
